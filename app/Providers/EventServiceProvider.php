<?php

namespace App\Providers;

use App\Events\Auth\UserLoginEvent;
use App\Events\Auth\UserLogoutEvent;
use App\Listeners\Auth\LogUserLogin;
use App\Events\Auth\LoginFailedEvent;
use App\Listeners\Auth\LogUserLogout;
use App\Listeners\Auth\LogLoginFailed;
use Illuminate\Auth\Events\Registered;
use App\Events\PostPublish\PostPublished;
use App\Events\PostPublish\PostPublishFailed;
use App\Listeners\PostPublish\LogPostPublished;
use App\Listeners\PostPublish\LogPostPublishFailed;
use Illuminate\Auth\Listeners\SendEmailVerificationNotification;
use Illuminate\Foundation\Support\Providers\EventServiceProvider as ServiceProvider;


class EventServiceProvider extends ServiceProvider
{
    /**
     * The event to listener mappings for the application.
     *
     * @var array<class-string, array<int, class-string>>
     */
    protected $listen = [
        // Auth
        UserLoginEvent::class => [
            LogUserLogin::class,
        ],
        UserLogoutEvent::class => [
            LogUserLogout::class,
        ],
        LoginFailedEvent::class => [
            LogLoginFailed::class,
        ],

        // Post Publish
        PostPublished::class => [
            LogPostPublished::class,
        ],
        PostPublishFailed::class => [
            LogPostPublishFailed::class,
        ],
    ];

    /**
     * Register any events for your application.
     */
    public function boot(): void
    {
        //
    }

    /**
     * Determine if events and listeners should be automatically discovered.
     */
    public function shouldDiscoverEvents(): bool
    {
        return false;
    }
}
